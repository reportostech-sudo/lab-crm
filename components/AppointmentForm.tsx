...
import { Calendar, User, Phone, Mail, FileText, CheckCircle, Loader2, MapPin } from "lucide-react";
...
export default function AppointmentForm({ doctorName }: { doctorName?: string }) {
    ...
    if (state?.message === 'Success! Booking created.') {
        return (
            <div className="bg-green-50/50 p-10 rounded-3xl text-center border border-green-100 h-full flex flex-col justify-center items-center backdrop-blur-sm">
                ...
                <p className="text-gray-600 mb-6 max-w-xs mx-auto">
                    We have received your details. Our team will call you shortly to confirm.
                </p>
                <button
                    onClick={() => window.location.reload()}
                    className="text-white bg-green-500 px-6 py-2 rounded-full font-semibold hover:bg-green-600 transition"
                >
                    Book Another
                </button>
            </div>
    ...
    // Uses a cleaner, more integrated design without heavy shadows
    return (
        <form action={dispatch} className="bg-white p-8 md:p-10 rounded-3xl md:shadow-none h-full md:bg-transparent">
                ...
                <div>
                    ...
                    <div className="relative group">
                        ...
                        <input
                            ...
                            name="date"
                            required
                            className="w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-medical-teal-500 focus:bg-white focus:border-transparent outline-none transition-all font-medium text-gray-700"
                        />
                    </div>
                    {state?.errors?.date && <p className="text-red-500 text-xs mt-1">{state.errors.date}</p>}
                </div>

                {/* Service Type Selection - Moved Up */}
                <div>
